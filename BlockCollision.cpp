...
void generateblocks(float **matrix, long *keymatrix){
...
void FindElementsWithinDia(float **matrix)
{
    ...
    for(int j=0; j<500; j++)
    {
      for(int k=0; k<10; k++)
      {
        ...
        for(int l=0; l<10; l++)
        {
          ...
          {
            if(m ==l || m == k) continue;
            int thirdelement = cells[i][j].cell[l];
            for(int n=0;n<10; n++)
            {
              if(n == m || n == l || n == k) continue;
              int fourthelement = cells[i][j].cell[n];
              newblocks[i].column = j;
              newblocks[i].signature = keymatrix[firstelement] +
              keymatrix[secondelement] +
              keymatrix[thirdelement] + keymatrix[fourthelement];
...
void findcollisions(newBlocks *newblocks)
{
  for(int i=0; i<1000000;i++)
  {
    for(int j=0; j<10000;j++)
    {
      ...
      {
        col[i].signature = newblocks[i].signature;
        for(int k=0; k<4; k++)
        {
          col[i].collisionblocks[k] = newblocks->elements[k];
        }
      }
    }
  }
